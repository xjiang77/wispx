...
|-----------|-------|--------|-------|
| **Eval Run** | skill + eval prompt + files | transcript, outputs, metrics | `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/executor.md` |
| **Grade Expectations** | outputs + expectations | pass/fail per expectation | `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/grader.md` |
| **Blind Compare** | output A, output B, eval prompt | winner + reasoning | `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/comparator.md` |
| **Post-hoc Analysis** | winner + skills + transcripts | improvement suggestions | `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/analyzer.md` |
...
See `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/references/mode-diagrams.md` for detailed visual workflow diagrams.

---

For task tracking patterns (lifecycle, creating tasks, updating stages), see `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/references/task-tracking.md`.
...
|-------|------|-----------|
| **Executor** | Run skill on a task, produce transcript + outputs + metrics | `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/executor.md` |
| **Grader** | Evaluate expectations against transcript and outputs | `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/grader.md` |
| **Comparator** | Blind A/B comparison between two outputs | `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/comparator.md` |
| **Analyzer** | Post-hoc analysis of comparison results | `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/analyzer.md` |
...
You can initialize with `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/scripts/init_json.py evals evals/evals.json` and validate with `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/scripts/validate_json.py evals/evals.json`. See `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/references/schemas.md` for the full schema.
...
After creation is complete, tell the user the skill directory path so they can install or share it.
...
0. **Read output schemas**:
   ...
   ```bash
   Read ${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/references/schemas.md  # JSON structures for grading, history, comparison, analysis
   ```
...
2. **Copy skill to v0**:
   ```bash
   ${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/scripts/copy_skill.py <skill-path> <skill-name>-workspace/v0 --iteration 0
   ```

3. **Verify or create evals**:
   - Check for existing `evals/evals.json`
   - If missing, ask user for 2-3 example tasks and create evals
   - Use `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/scripts/init_json.py evals` to create with correct structure
...
5. **Initialize history.json**:
   ...
   ```bash
   ${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/scripts/init_json.py history <workspace>/history.json
   ```

   Then edit to fill in skill_name. See `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/references/schemas.md` for full structure.
...
```
Read ${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/executor.md
...
```
Read ${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/grader.md
...
```
Read ${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/comparator.md
...
```
Read ${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/analyzer.md
...
1. Copy current best to new version:
   ```bash
   ${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/scripts/copy_skill.py workspace/<current_best>/skill workspace/v<N+1> \
       --parent <current_best> \
       --iteration <N+1>
   ```
...
Tell the user the skill directory path so they can install or share it.
...
- **Inline execution**: Read `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/executor.md` and follow the procedure directly in your main loop. Then read `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/grader.md` and follow it directly to grade the results.
...
```
Read ${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/references/eval-mode.md      # Complete Eval workflow
Read ${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/references/schemas.md        # JSON output structures
```
...
Without subagents, execute and grade sequentially in the main loop. Read the agent reference files (`${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/executor.md`, `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/grader.md`) and follow the procedures directly.
...
```
Read ${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/references/benchmark-mode.md # Complete Benchmark workflow
Read ${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/references/schemas.md        # JSON output structures
```
...
For workspace directory layouts and key file descriptions, see `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/references/workspace-structure.md`.
...
**Without subagents**: Read the agent reference file (e.g., `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/executor.md`) and follow the procedure directly in your main loop. Execute each step sequentially — the procedures are designed to work both as subagent instructions and as inline procedures.
