...
2. Prepare (${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/scripts/prepare_eval.py)
   → Create task, copies skill, stages files

3. Execute (${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/executor.md)
   → Update task to implementing, spawn executor sub-agent
   → Executor reads skill, runs prompt, saves transcript

4. Grade (${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/grader.md)
...
Read ${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/references/schemas.md
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/scripts/prepare_eval.py <skill-path> <eval-id> --output-dir <workspace>/eval-<id>/
```
...
**Without subagents**: Read `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/executor.md` and follow the procedure directly — execute the eval, save the transcript, and produce outputs inline.
...
**Without subagents**: Read `${CLAUDE_PLUGIN_ROOT}/skills/skill-creator/agents/grader.md` and follow the procedure directly — evaluate expectations against the transcript and outputs, then save grading.json.
